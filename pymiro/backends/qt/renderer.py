...
from pymiro.backends.qt.widgets import apply_prop, create_widget, remove_prop
...
class QtRenderer(QObject):
    ...
    def update(self, node_id: str, changed: dict[str, Any], removed: list[str]) -> None:
    ...
    def destroy(self, node_id: str) -> None:
        ...
        events_to_unbind = [
            evt for (nid, evt) in list(self.event_connections.keys()) if nid == node_id
        ]
    ...
    def set_children(self, parent_id: str, child_ids: list[str]) -> None:
        ...
        is_grid = (
            hasattr(layout, "addWidget") and parent.property("grid_cols") is not None
        )
    ...
    def move(self, node_id: str, new_parent_id: str) -> None:
        ...
        if new_layout is not None:
            is_grid = (
                hasattr(new_layout, "addWidget")
                and new_parent.property("grid_cols") is not None
            )
            if is_grid:
    ...
    def bind_event(self, node_id: str, event: str, handler: Any) -> None:
        ...
        if conn is not None:
            self.event_connections[(node_id, event)] = {
                "conn": conn,
                "wrapper": wrapper,
                "state": state,
            }
    ...
    def commit(self, patches: list[Patch]) -> None:
        ...
        QMetaObject.invokeMethod(
            self, "_apply_patches", Qt.ConnectionType.QueuedConnection
        )
