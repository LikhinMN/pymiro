...
def _get_hint(exc_type: type[BaseException], exc_value: BaseException) -> list[str]:
    ...
    if name == "ComponentError":
        ...
        return ["Hint: a computed value cannot depend on itself"]
    elif "key" in str(exc_value).lower():
        # Handle "Missing key warn" or duplicate key errors
        return ["Hint: add key= props to list children"]
    return []
...
def pymiro_excepthook(
    ...
    name = exc_type.__name__
    is_pymiro_error = (
        name in ("ComponentError", "CycleError")
        or "pymiro" in str(exc_type)
        or "key" in str(exc_value).lower()
    )
    ...
    if is_pymiro_error:
        ...
        title = "pymiro error"
        lines = [f"{name} in {func_name}", str(exc_value), ""]
